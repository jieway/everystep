...
跑一遍 MapReduce 的词频统计
`src/main/mrsequential.go` 是一个单进程的 MR 实现，

除此之外还为你提供了几个 MapReduce 应用程序：`mrapps/wc.go`中的单词计数，以及`mrapps/indexer.go`中的文本索引器。使用方式如下：
...
一个开始的方法是修改 `mr/worker.go` 的Worker()，向协调者发送一个RPC，要求完成一个任务。
