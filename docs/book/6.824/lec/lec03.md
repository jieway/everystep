...
如果只有一个服务器，一旦发生故障整个系统就崩溃了。也就是单个服务器容错能力不行。实际中通常构建主从复制服务器。

假设有两份同样的数据分别保存在不同的两台服务器上，如果其中一台服务器故障那么可以直接切换到另一台服务器上读写。但是为了维护一致性，两台服务器都需要处理写入操作。而读取操作则是单个服务器处理，如果必须从两台服务器上读，一旦数据丢失就不行了。

假设此时有两台服务，两个客户端，两个客户端都有写入操作。那么每个客户端会同时向两个服务器发送写请求。但是因为服务器处理不同客户端的顺序不同进而会导致结果不一样。当其他客户端读取数据时就会出问题。但是对于单个服务器来说，这些问题都不会出现。为了解决这个问题需要服务器间进行很复杂的通信才能实现。GFS 以一种很好但不是很完美的方式解决了这个问题。

## 5. GFS

谷歌的业务场景迫使他们需要研究出来一种高速并行的方式来访问这些海量数据。此外还要一种全局的文件系统使得不同的程序都能从中获取数据。此时需要一个通用且全局可复用的存储系统。

为了加快读写速度，GFS 将数据自动拆分到多个服务器中，然后并行读写。此外 GFS 是运行在单个数据中心的系统，并且 GFS 不开源。适合对大型数据连续访问而非随机访问的场景。GFS 并没有在网络延迟上做很多优化，重心是在吞吐上。

GFS 容忍了弱一致性的存在，也就是并没有保证返回的是正确数据。舍弃这点获得了更好的性能。论文不局限单个 master 节点，存在多个 master 也能很好的工作。但是多个 master 会减弱一致性。

允许弱一致性存在的场景，例如通过搜索引擎得到的几万条结果中丢失了几条，或者说顺序出错，这完全是可以接收的。但是在一些金融场景下则完全不行。
