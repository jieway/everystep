...
正常而言，树的结点都是**一个键**和**两条链接**分别指向**子节点**。（注意后续提到键时默认也包含了该键所对应的值）
...
对于完美平衡的 2-3 查找树而言，从根节点出发到叶子节点的距离都是相同的。

### 查找

从根节点出发，当根结点的键和要查找的键相等时表示命中，也就是找到。

反之，如果要查找的键小于根结点的键，那么去左子树里面查找，反之去又子树里面查。

之上是常规的 2- 结点的处理方式，对于 3- 结点需要和当前节点内的两个键比较。

<div align="center"><img src="https://gitee.com/weijiew/pic/raw/master/img/20200715010622.png"/></div>

如果要查找的键小于当前节点内的两个键那么就去该节点的左子树里面查找，反之要查找的键均大于节点内的两个键，那么就去右子树里面查找。

因为又三条链接，所以当要查找的键大于其中一个节点且小于另外一个节点时就去中间的链接指向的子树里面查找。
...
此时根据查找操作遍历到了需要插入节点上，假设这个结点是 2- 结点，为了保持树的平衡性，可以将要插入的键插在该结点上将其变为 3- 结点。

### 向一颗**只含有** 3- 结点的树中插入新键

考虑一种特殊情况，也就是树中所有的结点均是 3- 结点。

当向 3- 结点插入新键时可以将其变为 4- 结点，也就是该节点存在 4 条链接和 3 个键。
...
4- 结点将中键向上**提升**，因为父节点是 3- 结点，提升后变成了 4- 结点。父节点继续向上提升中键。

从下之上的不断提升，这样不断的操作直到遇到了 2- 结点，将其变成了 3- 结点。或者始终没有遇到 2- 结点最终导致来到了根节点上，根节点变成了 4- 结点，起本身再继续提升为两个 2- 结点和一个 3- 结点。
...
换句话说，从根节点出发到任何叶子节点的路径长度都是相等的。

如图，这是一个 3- 节点中键向上提升的过程，但是变化前后，根节点到每一个叶子节点的路径长度没有发生改变。

<div align="center"><img src="https://gitee.com/weijiew/pic/raw/master/img/20200715145409.png"/></div>

只有根节点从 4- 结点变为三个 2- 结点时路径长度才会发生改变。从根结点到所有叶子结点的路径长度均加一。

但是这个改变也是针对所有叶子节点的改变，而不是针对某个结点，所以并没有打破平衡性。
...
如果将红黑树中的所有红链接都画平，那么说根节点到所有的叶子节点的距离都是相等的。
...
将结点的的数据结构写出来，注意需要表示出每个父节点指向当前节点的颜色。
...
先考虑简单的情况，向只有 2- 节点的红黑树中插入新键。

如果新键大于根节点的键，那么直接插在左子树上，左连接变红。变成了 3- 结点。
...
复杂在于当删除节点时需要临时构造 4- 结点且需要不断的向下变换，之后还需要向上将临时构造的 4- 结点再分解掉，而此时同插入操作就类似了。
...
> 在大小为 $N$ 的红黑树中，根节点到任意结点的平均路径长度为 $~1.001lgN$。
...
对于重复频率查区频率比较高的键，hashCode() 会将其缓存下来，后续使用时不用再去计算。
